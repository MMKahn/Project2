...
First, we will pull data from the API to create new tibbles `CO_Brew` with 50 breweries in the state of Colorado and `CA_Brew` with 50 breweries in the state of California using the `Get_OB_DataFrame()` function to contact, query, and parse the API data. 

```{r}
CO_Brew <- Get_OB_DataFrame(size = 50, search_by = "state", input = "Colorado")
CO_Brew
...
The code chunk below creates a one-way contingency table showing the number of each type of brewery (from the `brewery_type` variable) in the data set with 50 random breweries selected in Colorado (`CO_Brew`) using the `table()` function. 

This contingency table indicates that there are 17 brewpubs, 2 contract, 3 large, 21 micro, 4 planning, 2 proprietor, and 1 regional brewery in the Colorado data.

```{r}
table(CO_Brew$brewery_type)
```

The code chunk below creates a two-way contingency table showing the number of each type of brewery (from the `brewery_type` variable) per `city` for the 50 random breweries selected in Colorado (`CO_Brew`) using the `table()` function. 

This contingency table indicates that Denver (the 10th column) has the most breweries in this data set with 4 brewpubs, 2 large, 4 micro, 2 planning, and 2 proprietor breweries.

```{r}
table(CO_Brew$brewery_type, CO_Brew$city)
```
